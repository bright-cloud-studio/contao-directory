...
class Handler
{
    ...
    public function onPrepareForm($submittedData, $formData, $files, $labels, $form)
    {
        ...
        if($formData['formID'] == 'directory_submission') {
            ...
            // Create a LOG so we can tell if this is working or not
            $myfile = fopen($_SERVER['DOCUMENT_ROOT'] . '/../files/content/directory/logs/prepare_form_'.strtolower(date('m_d_y_H:m:s')).".txt", "w") or die("Unable to open file!");
            foreach($submittedData as $key => $val) {
                // Write the JSON values to the log
                fwrite($myfile, "Key: " . $key . "  | Value: " . $val . "\n");
            }
            ...
            <html>
            <head>
            <title>[DD] Directory - Before Processing</title>
            </head>
            <body>
            <p>This is a listing BEFORE it is being processed. If there is an error, we should still receive this email</p>
            <h2>Listing Details:</h2>
            ";
            $message_user_contents = "";
            $message_end = "
